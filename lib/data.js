...
// Amenity & Specification are referenced via ObjectId refs on Project. Their
// modules must be evaluated so `.populate('amenities'|'specifications')` can
// resolve the schema — otherwise it throws MissingSchemaError and the detail
// page silently falls back to seed data / unpopulated ids.
...
export { toNextMetadata, resolvePageMetadata, resolveProjectsPageMetadata } from './pageMetadata';
...
/** SEO metadata for a CMS-managed static page (reads Page doc + defaults). */
export async function getPageMetadata(slug) {
  try {
    await connectDB();
    const [doc, settings] = await Promise.all([
      Page.findOne({ slug }).lean(),
      slug === 'home' ? getSettings() : Promise.resolve(null),
    ]);
    return resolvePageMetadata(slug, doc, settings);
  } catch {
    return resolvePageMetadata(slug, null, null);
  }
}

/** SEO metadata variants for /projects (all, residential, commercial). */
export async function getProjectsPageMetadata() {
  try {
    await connectDB();
    const doc = await Page.findOne({ slug: 'projects' }).lean();
    const sections = await getPageSections('projects');
    return resolveProjectsPageMetadata(doc, sections);
  } catch {
    return resolveProjectsPageMetadata(null, DEFAULT_PAGE_SECTIONS.projects || {});
  }
}

/** Next.js Metadata object for a CMS page slug. */
export async function getPageNextMetadata(slug) {
  const meta = await getPageMetadata(slug);
  return toNextMetadata(meta, { absoluteTitle: meta.absoluteTitle });
}
...
/* ── Hero Slides ──
 * Prefer admin-curated banners from the HeroSlide collection. When none exist,
 * fall back to the latest project thumbnails so the slider is never empty. */
export async function getHeroSlides() {
  try {
    await connectDB();

    const banners = await HeroSlide.find({ isActive: true })
      .sort({ order: 1, createdAt: 1 })
      .lean();
    if (banners.length) {
