...
export function SearchBar() {
  const [filters, setFilters] = useState({ status: '', type: '', location: '', budget: '' });
  ...
  return (
    <div className="srch-root">
      <div className="container srch-inner">
        <div className="srch-fields">
          <div className="srch-field">
            ...
            <select className="srch-select" value={filters.status} onChange={e => set('status', e.target.value)}>
          ...
          <div className="srch-field">
            ...
            <select className="srch-select" value={filters.type} onChange={e => set('type', e.target.value)}>
              <option value="">Any</option>
          ...
          <div className="srch-field">
            ...
            <select className="srch-select" value={filters.location} onChange={e => set('location', e.target.value)}>
              <option value="">Any</option>
          ...
          <div className="srch-field">
            ...
            <select className="srch-select" value={filters.budget} onChange={e => set('budget', e.target.value)}>
              <option value="">Any</option>
      ...
      <style jsx global>{`
        .srch-root { background: var(--green,#006833); padding: 28px 0; box-shadow: 0 6px 24px rgba(0,104,51,0.25); }
        .srch-inner {}
        .srch-fields { display: flex !important; align-items: flex-end !important; gap: 12px; flex-wrap: wrap; }
        .srch-field { display: flex !important; flex-direction: column !important; gap: 6px; flex: 1; min-width: 140px; }
        .srch-label { font-size: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: 1.2px; color: rgba(255,255,255,0.7); }
        .srch-select { height: 44px; border: 1.5px solid rgba(255,255,255,0.25); border-radius: 6px; background: rgba(255,255,255,0.1); color: white; padding: 0 12px; font-size: 13.5px; font-weight: 500; cursor: pointer; transition: border-color 0.2s; appearance: none; backdrop-filter: blur(4px); }
        .srch-select:focus { outline: none; border-color: white; background: rgba(255,255,255,0.18); }
        .srch-select option { background: var(--green-dark,#004d26); color: white; }
        .srch-btn { display: inline-flex !important; align-items: center !important; justify-content: center !important; gap: 8px; height: 44px; padding: 0 28px; background: var(--red,#eb3237); color: white; border-radius: 6px; font-size: 13px; font-weight: 800; text-decoration: none; white-space: nowrap; transition: all 0.2s; flex-shrink: 0; letter-spacing: 0.3px; }
        .srch-btn:hover { background: var(--red-dark,#c01a1e); transform: translateY(-1px); }
        @media(max-width:768px){ .srch-fields{flex-direction:column;} .srch-field{min-width:100%;} }
      `}</style>
    </div>
  );
}
...
export function CertificationsSection({ settings = {} }) {
  const heading      = settings.cert_heading   || 'RERA Registered Projects & Member of CREDAI';
  ...
  return (
    <section className="cert-root">
            ...
            <div className="cert-partners">
              {partnerLogos.map((src, i) => (
                <div key={i} className="cert-partner-item">
                  <img src={src} alt={`Partner ${i + 1}`} className="cert-partner-img" />
                </div>
              ))}
            </div>
          </>
      ...
      <style jsx global>{`
        ...
        .cert-logo-img:hover { transform: scale(1.04); }
        ...
        .cert-partner-item:hover { background: #f9fafb; }
        .cert-partner-img {
          max-height: 105px;
          max-width: 145px;
          width: auto;
        ...
        @media (max-width: 768px) {
          .cert-logos { gap: 32px; }
          .cert-logo-img { max-height: 80px; max-width: 180px; }
          .cert-partner-img { max-height: 38px; max-width: 110px; }
          .cert-partners { gap: 8px 16px; }
        }
      `}</style>
    </section>
  );
...
export function TrustBanner({ settings = {} }) {
  ...
  return (
    <div className="tb-root">
      ...
      <style jsx global>{`
        .tb-root { background: #fff; padding: 72px 0; border-bottom: 1px solid #f0f0f0; }
        .tb-inner { display: grid !important; grid-template-columns: 1.2fr 1fr; gap: 72px; align-items: center; }
        .tb-tag { display: inline-block; background: var(--red-pale,#fef2f2); color: var(--red,#eb3237); font-size: 11px; font-weight: 800; text-transform: uppercase; letter-spacing: 2px; padding: 6px 16px; border-radius: 4px; margin-bottom: 16px; }
        .tb-title { font-size: clamp(32px,4vw,52px); font-weight: 900; color: #111; line-height: 1.1; margin-bottom: 20px; }
        .tb-desc { font-size: 15px; color: #555; line-height: 1.85; margin-bottom: 28px; }
...
export function AboutSection({ settings = {} }) {
  const img = settings.about_image || 'https://saviourgroup.in/wp-content/uploads/2025/05/about.png';
  ...
  const subtitle = settings.about_subtitle || 'Building dreams and delivering excellence across Delhi-NCR since 1999.';
  const desc1 = settings.about_desc_1 || 'M/s Saviour Builders Pvt. Ltd. (Saviour Group) is one of the leading real estate developers in Delhi-NCR.';
  const desc2 = settings.about_desc_2 || 'From building small residential projects to creating multi-dimensional mega townships, the Saviour Group has done it all.';
  const points = (settings.about_points || 'Quality Policy\nOn-Time Delivery\nTransparent Dealings\nAfter-Sales Support').split('\n').map(p => p.trim()).filter(Boolean);
  const ctaLabel = settings.about_cta_label || 'Read Our Story';
  const ctaUrl = settings.about_cta_url || '/about-us';

  return (
    <section className="as-root" style={{ paddingBottom: '0' }}>
  ...
  const cards = [
    {
      ...
      text: settings.mission_desc || 'To provide an outstanding level of Houses & service to our buyers, dedicated to the highest standards and performance necessary to fulfill all of your real estate dreams.',
    },
    {
      cls: 'mv-vision',
      icon: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>,
      iconCls: 'mv-icon-red',
      title: settings.vision_title || 'Our Vision',
      text: settings.vision_desc || 'We believe real estate is more than just building four walls — it\'s about "Building a Better Life".',
    },
    {
      cls: 'mv-quality',
      ...
      text: settings.quality_desc || 'We are committed to delight our customers by improving their quality of life through ethical practices and continuous improvement.',
    },
  ];
  return (
    <section className="mv-root">
      ...
      <style jsx global>{`
        .mv-root { padding: 80px 0; background: #fff; }
        .mv-grid { display: grid !important; grid-template-columns: repeat(3,1fr); gap: 28px; }
        .mv-card { padding: 40px 32px; border-radius: 14px; background: #fafafa; border: 1px solid #eee; transition: all 0.3s; }
        .mv-card:hover { box-shadow: 0 16px 48px rgba(0,0,0,0.1); transform: translateY(-6px); }
        .mv-mission:hover { border-color: var(--green,#006833); }
        .mv-vision:hover { border-color: var(--red,#eb3237); }
        .mv-quality:hover { border-color: var(--green,#006833); }
        .mv-icon-wrap { width: 64px; height: 64px; border-radius: 14px; display: flex !important; align-items: center !important; justify-content: center !important; margin-bottom: 20px; }
        .mv-icon-green { background: var(--green-pale,#e8f5ee); color: var(--green,#006833); }
        .mv-icon-red { background: var(--red-pale,#fef2f2); color: var(--red,#eb3237); }
        .mv-title { font-size: 20px; font-weight: 800; color: #111; margin-bottom: 14px; }
        .mv-text { font-size: 14.5px; color: #555; line-height: 1.8; }
        @media(max-width:768px){ .mv-grid{grid-template-columns:1fr; gap:20px;} }
      `}</style>
    </section>
  );
}
...
export function WhyUsSection({ settings = {} }) {
  ...
  return (
    <section className="wu-root">
      ...
      <style jsx global>{`
        .wu-root { padding: 96px 0; background: linear-gradient(160deg,var(--dark-900,#020c05) 0%,var(--dark-800,#041208) 60%,var(--dark-700,#062010) 100%); }
        .wu-grid { display: grid !important; grid-template-columns: repeat(3,1fr); gap: 24px; }
        .wu-card { padding: 40px 32px; border-radius: 14px; background: rgba(255,255,255,0.04); border: 1px solid rgba(255,255,255,0.08); transition: all 0.3s; position: relative; overflow: hidden; }
        .wu-card:hover { background: rgba(255,255,255,0.08); transform: translateY(-6px); }
        .wu-green:hover { border-color: rgba(0,104,51,0.5); }
        .wu-red:hover { border-color: rgba(235,50,55,0.5); }
        .wu-num { font-size: 64px; font-weight: 900; color: rgba(255,255,255,0.04); position: absolute; top: 12px; right: 20px; line-height: 1; }
        .wu-icon { font-size: 40px; margin-bottom: 18px; display: block; }
        .wu-title { font-size: 20px; font-weight: 800; color: white; margin-bottom: 14px; }
        .wu-desc { font-size: 14px; color: rgba(255,255,255,0.55); line-height: 1.8; margin-bottom: 24px; }
        .wu-line { height: 3px; border-radius: 2px; width: 40px; }
        .wu-line-green { background: var(--green,#006833); }
        .wu-line-red { background: var(--red,#eb3237); }
        @media(max-width:1024px){ .wu-grid{grid-template-columns:repeat(2,1fr) !important;} }
        @media(max-width:640px){ .wu-grid{grid-template-columns:1fr !important;} }
      `}</style>
    </section>
  );
}
  ...
  return (
    <div className="ls-root">
      ...
      <style jsx global>{`
        .ls-root {
          padding: 80px 0;
          background: linear-gradient(135deg, #f0faf5 0%, #e8f5ee 40%, #fef2f2 100%);
          position: relative;
          overflow: hidden;
        }
        .ls-root::before {
          ...
          border-radius: 50%;
          background: radial-gradient(circle, rgba(0,104,51,0.08) 0%, transparent 70%);
          pointer-events: none;
        }
        .ls-root::after {
          content: '';
          ...
          border-radius: 50%;
          background: radial-gradient(circle, rgba(235,50,55,0.07) 0%, transparent 70%);
          pointer-events: none;
        }
        .ls-outer { position: relative; z-index: 1; }
        .ls-glass {
          ...
          max-width: 860px;
          margin: 0 auto;
          box-shadow: 0 8px 48px rgba(0,104,51,0.1), 0 2px 8px rgba(0,0,0,0.04);
        }
        .ls-badge {
        ...
        .ls-title {
          ...
          color: #111; line-height: 1.15; margin-bottom: 18px;
        }
        .ls-title span { color: var(--green,#006833); }
        .ls-sub {
        ...
        .ls-btn-primary {
          ...
          padding: 14px 32px;
          background: linear-gradient(135deg,var(--green,#006833),var(--green-dark,#004d26));
          color: white; border-radius: 8px; font-weight: 700; font-size: 14px;
          text-decoration: none; transition: all 0.25s;
          box-shadow: 0 6px 20px rgba(0,104,51,0.3);
        }
        .ls-btn-primary:hover { transform: translateY(-2px); box-shadow: 0 12px 32px rgba(0,104,51,0.4); }
        .ls-btn-outline {
          display: inline-flex !important; align-items: center !important;
          padding: 14px 32px;
          border: 2px solid var(--green,#006833); color: var(--green,#006833);
          border-radius: 8px; font-weight: 700; font-size: 14px;
          text-decoration: none; transition: all 0.25s; background: transparent;
        }
        .ls-btn-outline:hover { background: var(--green,#006833); color: white; transform: translateY(-2px); }
        @media(max-width:640px){ .ls-glass{padding:40px 24px; border-radius:16px;} }
      `}</style>
    </div>
  );
}
...
export function DevelopmentsSection({ settings = {} }) {
  const segments = [
    {
      ...
      img: settings.dev_1_image || 'https://saviourgroup.in/wp-content/uploads/2024/11/SAVVIOUR-1-1.png',
    ...
    },
    {
      ...
      img: settings.dev_2_image || 'https://saviourgroup.in/wp-content/uploads/2024/11/mart.png',
    ...
    },
    {
      ...
      img: settings.dev_3_image || 'https://saviourgroup.in/wp-content/uploads/2025/05/about.png',
  ...
  return (
    <section className="dev-root">
          ...
          {segments.map(s => (
            <div key={s.title} className="dev-card">
              <div className="dev-img-wrap">
                <Image src={s.img} alt={s.title} fill className="dev-img" sizes="(max-width:768px) 100vw, 33vw" />
      ...
      <style jsx global>{`
        .dev-root { padding: 96px 0; background: white; }
        .dev-grid { display: grid !important; grid-template-columns: repeat(3,1fr); gap: 28px; }
        .dev-card { border-radius: 14px; overflow: hidden; box-shadow: 0 4px 24px rgba(0,0,0,0.08); border: 1px solid #eee; transition: transform 0.3s, box-shadow 0.3s; }
        .dev-card:hover { transform: translateY(-8px); box-shadow: 0 20px 56px rgba(0,0,0,0.14); }
        .dev-img-wrap { position: relative; height: 220px; overflow: hidden; }
        .dev-img { object-fit: cover; transition: transform 0.5s; }
        .dev-card:hover .dev-img { transform: scale(1.06); }
        .dev-img-overlay { position: absolute; inset: 0; background: linear-gradient(to bottom, transparent 40%, rgba(2,45,18,0.65) 100%); }
        .dev-tag { position: absolute; bottom: 14px; left: 14px; padding: 4px 12px; border-radius: 4px; font-size: 10px; font-weight: 800; text-transform: uppercase; letter-spacing: 0.8px; }
        .dev-tag-green { background: var(--green,#006833); color: white; }
        .dev-tag-red { background: var(--red,#eb3237); color: white; }
        .dev-body { padding: 24px; }
        .dev-title { font-size: 19px; font-weight: 800; color: #111; margin-bottom: 12px; line-height: 1.3; }
        .dev-desc { font-size: 13.5px; color: #555; line-height: 1.75; margin-bottom: 20px; }
        .dev-btn { display: inline-flex !important; align-items: center !important; gap: 8px; padding: 10px 22px; border-radius: 6px; font-size: 13px; font-weight: 700; text-decoration: none; transition: all 0.2s; }
        .dev-btn-green { background: var(--green-pale,#e8f5ee); color: var(--green,#006833); }
        .dev-btn-green:hover { background: var(--green,#006833); color: white; }
        .dev-btn-red { background: var(--red-pale,#fef2f2); color: var(--red,#eb3237); }
        .dev-btn-red:hover { background: var(--red,#eb3237); color: white; }
...
export function BlogSection({ posts, settings = {} }) {
  return (
    <section className="bls-root">
      ...
      <style jsx global>{`
        .bls-root { padding: 96px 0; background: #f7f9f8; }
        .bls-header { display: flex !important; align-items: flex-end !important; justify-content: space-between; margin-bottom: 40px; }
        .bls-header .sh-wrap { margin-bottom: 0; }
        .bls-all-link { display: inline-flex !important; align-items: center !important; gap: 6px; font-size: 13px; font-weight: 700; color: var(--green,#006833); text-decoration: none; white-space: nowrap; padding-bottom: 4px; border-bottom: 2px solid var(--green-pale,#e8f5ee); transition: border-color 0.2s; }
        .bls-all-link:hover { border-color: var(--green,#006833); }
        .bls-grid { display: grid; grid-template-columns: repeat(3,1fr); gap: 28px; }
        @media(max-width:1024px){ .bls-grid{grid-template-columns:repeat(2,1fr);} .bls-header{flex-direction:column; align-items:flex-start; gap:12px;} }
        @media(max-width:640px){ .bls-grid{grid-template-columns:1fr; gap:20px;} }
      `}</style>
    </section>
  );
}
...
export function TestimonialsSection({ testimonials, settings = {} }) {
  return (
    <section className="ts-root">
      ...
      <style jsx global>{`
        .ts-root { padding: 96px 0; background: linear-gradient(160deg,var(--dark-700,#062010) 0%,var(--dark-800,#041208) 100%); }
        ...
        @media(max-width:640px){ .ts-grid{grid-template-columns:1fr; gap:20px;} }
      `}</style>
    </section>
  );
}
...
export function ContactSection({ settings = {} }) {
  ...
  return (
    <section className="cs-root">
        ...
        <div className="cs-info">
          ...
          <ul className="cs-list">
            {[
              ...
              { icon: '📞', label: 'Mobile', val: phone2, href: `tel:${phone2.replace(/\s/g,'')}` },
              ...
              <li key={item.label} className="cs-item">
                ...
                <div>
                  ...
                  {item.href
                    ? <a href={item.href} className="cs-item-val">{item.val}</a>
      ...
      <style jsx global>{`
        .cs-root { padding: 96px 0; background: linear-gradient(135deg,var(--green-deep,#022d12) 0%,var(--dark-800,#041208) 100%); }
        .cs-inner { display: grid; grid-template-columns: 1fr 1fr; gap: 64px; align-items: start; }
        .cs-desc { font-size: 15px; color: rgba(255,255,255,0.6); line-height: 1.85; margin-bottom: 36px; }
        .cs-list { list-style: none; display: flex; flex-direction: column; gap: 18px; }
        .cs-item { display: flex !important; align-items: flex-start !important; gap: 14px; }
        .cs-icon { font-size: 22px; flex-shrink: 0; }
        .cs-item-label { font-size: 10px; text-transform: uppercase; letter-spacing: 1.2px; color: var(--red,#eb3237); margin: 0 0 2px; }
        .cs-item-val { font-size: 14.5px; color: rgba(255,255,255,0.75); text-decoration: none; transition: color 0.15s; }
        a.cs-item-val:hover { color: white; }
        .cs-form-card { background: white; border-radius: 16px; padding: 40px 36px; box-shadow: 0 24px 72px rgba(0,0,0,0.35); }
        .cs-form-title { font-size: 22px; font-weight: 800; color: #111; margin-bottom: 28px; padding-bottom: 16px; border-bottom: 3px solid var(--red,#eb3237); display: inline-block; }
        @media(max-width:1024px){ .cs-inner{grid-template-columns:1fr; gap:48px;} }
      `}</style>
    </section>
  );
}
