...
export default function SpecificationsPage() {
  ...
  useEffect(() => {
    fetch('/api/admin/specifications')
      .then((r) => r.json())
      .then((d) => { setItems(d.specifications || []); setLoading(false); })
  ...
  const handleAdd = async (e) => {
    ...
    try {
      ...
      setItems((prev) => [...prev, data.specification]);
      setForm(EMPTY_FORM);
      toast({ message: 'Specification added', type: 'success' });
    } catch (err) {
  ...
  const handleEditSave = async (id) => {
    try {
      const res = await fetch(`/api/admin/specifications/${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error);
      setItems((prev) => prev.map((s) => (s._id === id ? data.specification : s)));
      setEditing(null);
  ...
  const handleDelete = async () => {
    setDeleting(true);
    try {
      await fetch(`/api/admin/specifications/${confirm._id}`, { method: 'DELETE' });
      setItems((prev) => prev.filter((s) => s._id !== confirm._id));
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Manage project specification items with icons</p>
        </div>
      </div>

      <div className="layout">
        <div className="card">
          <h2 className="card-title">Add Specification</h2>
          <form onSubmit={handleAdd} className="add-form">
            <div className="form-group">
              <label>Specification Name *</label>
              <input type="text" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="e.g. Vitrified Flooring" required />
            </div>
            <div className="form-group">
              <label>Icon (react-icons name)</label>
              <IconPicker value={form.icon} onChange={(icon) => setForm({ ...form, icon })} />
            </div>
            <button type="submit" className="btn-primary" disabled={adding}>{adding ? 'Adding...' : '+ Add Specification'}</button>
          </form>
        </div>

        <div className="card">
          <h2 className="card-title">All Specifications <span className="count">({items.length})</span></h2>
      ...
      <ConfirmModal isOpen={!!confirm} title="Delete Specification?" message={`Delete "${confirm?.name}"?`} onConfirm={handleDelete} onCancel={() => setConfirm(null)} loading={deleting} />
      ...
      <style jsx global>{`
        ...
        .item-icon-box { background: #dbeafe; border-radius: 6px; padding: 6px 8px; margin-bottom: 6px; display: inline-block; }
        .icon-label { font-family: monospace; font-size: 11px; color: #1d4ed8; font-weight: 600; }
